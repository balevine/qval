...
- The UI disables the frozen Settings sections with a "locked — start a New evaluation to change" notice. The escape hatch is **New evaluation** (below), which unloads the file so everything is editable again.
...
- **OPEN** — one native open dialog that **auto-detects** the file: a Qbort `tickets.json` starts a **new working eval** (fresh evals, dataset fingerprint computed, schema/rules from current settings); a `*.qval.json` **loads that working file** (and its dataset must be re-importable or the tickets travel with it — see below), **hydrating** the working schema/rules and provider/model from the file's snapshot (§3). Defaults the dialog to `defaultDir`.
- **NEW EVALUATION** — a top-bar action that unloads the current (possibly locked) working file and opens a **`tickets.json`-only** picker, creating a fresh, fully-editable working file from the current settings. This is the sanctioned way to start scoring under a new schema/rules/model; the first save prompts for a new filename so the previous eval file isn't overwritten.
...
│     │  ├─ TopBar.tsx            # title · NEW · OPEN · EVALUATE · MERGE · EXPORT · gear (file actions disabled during a run)
...
- `session.open()` (auto-detects `tickets.json` vs `*.qval.json`) / `newEvaluation()` (fresh eval from a `tickets.json`) / `loadLast()` / `save()`
...
Single page:
- **Top bar**: title left; right: `NEW`, `OPEN`, `EVALUATE`, `MERGE`, `EXPORT`, gear (settings). `NEW` starts a fresh evaluation from a `tickets.json` (§10); `EVALUATE` is the primary (solid) action.
