...
/** Mirrors `DEFAULT_BATCH_SIZE` in src/shared/evaluation.ts. */
const DEFAULT_BATCH_SIZE = 10
/** Recorded on the evaluator so the app can tell a skill-produced file from an in-app run. */
const PROVIDER = 'claude-code'
/** Cosmetic `meta.appVersion`. The skill folder is copyable, so it can't read the app's package.json. */
...
/**
 * Row-level validation for a hand-authored schema file. `propertyErrors` is the app's editor
...
async function loadTickets(ticketsPath) {
  ...
  if (!parsed) {
    fail(`BAD_TICKETS ${ticketsPath}`, '  No usable tickets found (expected a tickets.json produced by Qbort).')
  }
  return parsed
}
...
/** `<dataset stem>.qval.json` in the working directory (mirrors the app's `suggestEvalName`). */
 ...
 * The app writes the same file on every human edit, so without this guard an `assemble` could
 * overwrite a human evaluation made while the run was out with the subagents.
 */
async function loadRunEvalFile(ctx) {
  ...
  if (file.meta.updatedAt !== ctx.evalUpdatedAt) {
    fail(
      ...
      '  It changed on disk since this run was planned (the app may have it open).',
      '  Close it in the app and re-run `plan`. Writing now would overwrite those edits.'
    )
  }
  return file
}
...
function buildRound(ctx, round, targets, rules, schema, previous) {
  const batches = chunk(targets, ctx.batchSize).map((batchTickets, index) => {
    ...
    // rather than sent separately as the app does.
...
async function cmdPlan(args) {
  ...
  if (existsSync(evalFilePath)) {
    ...
    // Both refusals below carry the app's own wording (src/main/storage.ts), so the CLI and the
    // MERGE dialog explain an incompatible file the same way.
    ...
    // One model scores every ticket in a file (spec §3/§4). This is the CLI side of the app's
    // provider lock, and it fires on the same signal: a scored `llm` evaluator.
    const locked = lockedLlmProvider(file)
    if (locked && locked.provider !== PROVIDER) {
      fail(
        `PROVIDER_LOCKED ${evalFilePath}`,
        `  Already scored in the app with ${locked.provider ?? '(unknown)'} · ${locked.model ?? '(unknown)'}.`,
        '  Continue that run in the app, or start a new eval file with --eval-file <new path>.'
      )
    }
...
async function cmdRetry(args) {
  ...
  if (round !== 1) {
    fail(
      ...
      '  A run gets exactly one automatic validation retry (--round 1), matching the app (spec §6).',
